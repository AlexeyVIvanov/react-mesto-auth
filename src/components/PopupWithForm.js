  ...
  return(
    ...
    <div className="popup__container popup__container_theme_light">
      ...
      <form onSubmit={props.onSubmit} name={`${props.name}`} className="popup__form popup__form-edit-profile" >
        <h2 className="popup__title popup__title_theme_light">{props.title}</h2>
          {props.children}
        <button type="submit" className="popup__submit popup__submit-edit-profile popup__submit_theme_light">{props.buttonTitle}</button>
      </form>
    </div>
  </section>
  )
